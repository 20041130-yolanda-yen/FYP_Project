...
import com.example.fypproject.ml.ModelMetadata;
...
public class BrandColor extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2{

    private static final String TYPE = "BRAND & COLOR";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_brand_color);
        ...
        listener = new LocationListener() {
            ...
            public void onLocationChanged(android.location.Location location) {
                try {
                    Geocoder geocoder = new Geocoder(BrandColor.this);
    ...
    private void updateUI() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                output_text.setText("Car num: " + output + "\n\n"
                        + "Brand: " + brandName + "\n"
                        + "Color: " + colorsForDB
                );
    ...
    private void getText(Bitmap bitmap) {
        ...
        Task<Text> result = textRecognition.process(image).addOnSuccessListener(new OnSuccessListener<Text>() {
            ...
            public void onSuccess(Text visionText) {
                ...
                Toast.makeText(BrandColor.this, "Detected: " + text, Toast.LENGTH_LONG).show();
    ...
    // -- Get Brand -- //
    private void getBrand(Bitmap bitmap) {
        try {
            ModelMetadata model = ModelMetadata.newInstance(BrandColor.this);

            // Creates inputs for reference.
            TensorImage image = TensorImage.fromBitmap(bitmap);

            // Runs model inference and gets result.
            ModelMetadata.Outputs outputs = model.process(image);
            ModelMetadata.DetectionResult detectionResult = outputs.getDetectionResultList().get(0);

            // Gets result from DetectionResult.
            brandName = detectionResult.getCategoryAsString();
    ...
    private void getColor(Bitmap bitmap) {
        ...
        colorsForDB = colors.getColorName(red, green, blue);
    ...
    private void verifyData(String car_num) {
        firebaseDataHelper = new FirebaseDataHelper(BrandColor.this);
    ...
    private void updateSQL(String TYPE, String carNum, String location) {
        ...
        try {
            ...
            Toast.makeText(BrandColor.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            scannedVehicles = new ScannedVehicles(-1, "Error", "Error", "Error", "Error", "Error", "Error");
        }

        ScannedVehiclesHelper databasehelper = new ScannedVehiclesHelper(BrandColor.this);
        boolean success = databasehelper.addOne(scannedVehicles);

//        Toast.makeText(ColorBrand.this, "Saving scanned data: " + success, Toast.LENGTH_SHORT).show();
    }
    ...
    private void showDialog(String title){
        AlertDialog.Builder mBuilder = new AlertDialog.Builder(BrandColor.this);
        View mView = getLayoutInflater().inflate(R.layout.dialog_spinner, null);
        mBuilder.setTitle(title);
        Spinner sp1 = (Spinner) mView.findViewById(R.id.spinner);
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(BrandColor.this,
        ...
        mBuilder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if (!sp1.getSelectedItem().toString().equals("Edit location")) {
                    ...
                    Toast.makeText(BrandColor.this, "You have selected: " + carpark, Toast.LENGTH_SHORT).show();
                    locationManager.removeUpdates(listener);
                    updateUI();
                    dialog.dismiss();
                } else{
                    Toast.makeText(BrandColor.this, "No location change.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }
            }
        });
